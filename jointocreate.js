...
const HP = '856278623358812161' // ruolo custom che ho io 
...
//voice state update event to check joining/leaving channels
  ...
  if (oldState.channelID && newState.channelID) {
    ...
    async function jointocreatechannel(user) {
      ...
      await user.guild.channels.create(`Sala ${user.member.user.username}`, {
        type: 'voice',
        ...
        await vc.overwritePermissions([
          {
            id: user.id,
            allow: ['MANAGE_CHANNELS', 'MUTE_MEMBERS', 'MOVE_MEMBERS', 'PRIORITY_SPEAKER', 'STREAM', 'MANAGE_ROLES'],
          },
          {
            id: HP,
            allow: ['MANAGE_CHANNELS', 'MUTE_MEMBERS', 'MOVE_MEMBERS', 'PRIORITY_SPEAKER', 'STREAM', 'MANAGE_ROLES'],
